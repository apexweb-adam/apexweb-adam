...
# Usage: print-outage-status.sh [--watch SECONDS]
set -euo pipefail
...
run_outage_status() {
  local CODE_REV DASH_REV RUNNING SNAPSHOT CHECKLIST CME_CHECKLIST STATUS
...
print(
    f"US stocks: phase={d.get('phase')} open_ready={orx.get('symbols')} "
    f"burst={events.get('has_burst_scan')} auto_entry={events.get('has_auto_entry')} "
    f"outage_recovery_scan={events.get('has_outage_recovery_scan')}"
)
if outage.get("recovery_scan_pending_burst"):
    print("  platform_outage_recovery: recovery_scan_pending_burst=true")
...
print("Crypto: 24/7 — held positions get immediate post-outage scan on startup (r466+)")
PY
...
if [[ -n "$WATCH_INTERVAL" ]]; then
  while true; do
    run_outage_status || true
    echo ""
    echo "Watching outage status — next check in ${WATCH_INTERVAL}s (Ctrl+C to stop)"
