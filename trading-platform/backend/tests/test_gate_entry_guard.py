...
def test_commodities_verification_cap_pressure_wind_down():
  from app.engines.gate_entry_guard import (
...
def test_crypto_momentum_retreat_cooldown_bypass():
  ...
  assert crypto_momentum_retreat_cooldown_bypass(
    **retreat,
    open_count=2,
    shadow_open_cap=2,
    last_exit_reason="Shadow cap-pressure loser wind-down",
  ) is True
  assert crypto_momentum_retreat_cooldown_bypass(
...
def test_commodities_monday_open_ready_sticky_queue(weekend_commodities_session):
  ...
  assert commodities_monday_open_ready(**base) is False
  assert commodities_monday_open_ready(**base, sticky_queue=True) is True
  assert commodities_monday_open_ready(
    **{**base, "composite": COMMODITIES_GRADUATION_OPEN_COMPOSITE_FLOOR - 0.03},
    sticky_queue=True,
  ) is False
...
def test_commodities_reopen_imminent_scan_active():
  from app.engines.gate_entry_guard import (
    COMMODITIES_REOPEN_IMMINENT_SCAN_INTERVAL,
  ...
  assert commodities_effective_scan_interval(
...
def test_build_session_prep_status_reopen_imminent():
  from app.engines.gate_entry_guard import build_session_prep_status

  status = build_session_prep_status(
    stocks_session={"in_session": False, "minutes_until_open": 3000, "mode": "weekend_closed"},
    commodities_session={"in_session": False, "minutes_until_open": 20, "mode": "weekend_closed"},
    stocks_trade_count_nudge=True,
    commodities_graduation_nudge=True,
  )
  assert status["commodities"]["gate_reopen_imminent"] is True
...
def test_build_session_prep_status_verification_near_floor_gap():
  from app.engines.gate_entry_guard import (
    COMMODITIES_VERIFICATION_TRADE_COUNT_MIN_COMPOSITE,
    build_session_prep_status,
  )

  composite = COMMODITIES_VERIFICATION_TRADE_COUNT_MIN_COMPOSITE - 0.052
  status = build_session_prep_status(
    stocks_session={"in_session": False, "minutes_until_open": 2220, "mode": "outside_session"},
  ...
  assert status["commodities"]["near_floor_symbols"] == ["XAUUSDT"]
...
def test_commodities_proven_winner_profit_lock_eases_threshold():
  ...
  assert shadow_graduation_profit_lock(
    unrealized=2.1,
    **{**base, "symbol": "NG=F", "proven_winners": frozenset({"CL=F"}), "profit_factor": 1.35},
  ) is False


def test_commodities_graduation_pf_profit_lock_eases_threshold():
  from app.engines.gate_entry_guard import shadow_graduation_profit_lock
  ...
  assert shadow_graduation_profit_lock(unrealized=2.05, **base) is True
  assert shadow_graduation_profit_lock(unrealized=1.95, **base) is False
...
def test_crypto_momentum_retreat_raw_signal_ok():
  ...
  assert crypto_momentum_retreat_raw_signal_ok(
    CRYPTO_MOMENTUM_RETREAT_ALIGNED_RAW_SIGNAL + 0.02, **aligned
  ) is True
  assert crypto_momentum_retreat_raw_signal_ok(
