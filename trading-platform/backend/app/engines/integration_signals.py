...
async def get_integration_boost(session: AsyncSession, symbol: str) -> tuple[float, str]:
  ...
  for item in items[:5]:
    ...
    if item.source == "axiom":
      weight = 0.26 if item.relevance_score >= 0.88 else 0.20
    if item.source == "phantom":
      weight = 0.18
  ...
  if axiom_bullish and (dex_bullish or fomo_bullish):
    boost += 0.05
    reasons.append("axiom_wallet_confluence:+0.05")
